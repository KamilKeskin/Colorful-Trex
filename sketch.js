...
function draw() {
 ...
 text("Score: "+ score,30,50);
  ...
  if(gameState === PLAY){
    ...
    if(keyDown("space") || touches.lenght>0 && trex.y >= 416) {
        trex.velocityY = -8;       
        jumpSound.play();
      touches=[] 
    }  
    //touches is an array data type that stores all touched coordinates as a comma seperated list.
    //array is used with []s
    //The first ellement of an array is an index 0 and the last ellement is at the index= to the value of the arrays lenght property -1
